...
A synthetic document containing one long paragraph with many short footnote
references that splits across pages. Used to verify that references in split
paragraph fragments are mapped to the page containing their rendered line, so
the painted footnote area does not overlap body text.
